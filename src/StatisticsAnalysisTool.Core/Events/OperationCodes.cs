// Mirrored verbatim from src/StatisticsAnalysisTool/Network/OperationCodes.cs (upstream WPF app).
// The enum ORDINAL is the Photon wire operation code (parameter 253), so the order must match
// upstream exactly. Re-sync this file when upstream changes operation codes (game patches).
