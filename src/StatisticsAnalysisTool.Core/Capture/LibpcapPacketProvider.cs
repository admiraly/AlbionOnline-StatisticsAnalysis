...
// Adapted from the upstream WPF app's LibpcapPacketProvider. The only behavioural change is
// that capture settings (device selection, BPF filter) are taken from an injected
// CaptureOptions instead of the static SettingsController singleton, so this compiles and runs
// without any WPF / Windows dependency. libpcap is native to Linux; the same binding drives
// Npcap on Windows.
public class LibpcapPacketProvider : PacketProvider
{
    ...
    public LibpcapPacketProvider(IPhotonReceiver photonReceiver, AlbionServerDetectionService albionServerDetectionService, CaptureOptions options)
    {
    ...
    private int OpenConfiguredDevices(PcapDispatcher dispatcher, bool logDeviceDetails)
    {
        ...
        var configuredDevices = _options.NetworkDevices ?? new List<NetworkDeviceSelection>();
    ...
    {
        var user = _options.PacketFilter?.Trim();
